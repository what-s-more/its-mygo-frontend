...
import {
  ...
  type CommunityFavoritePostItem,
  type CommunityPost,
  type CommunityTopic,
...
export function UserTestConsolePage() {
  ...
  const [postProductSearchPage, setPostProductSearchPage] = useState(1)
  const [postProductSearchPageSize, setPostProductSearchPageSize] = useState(8)
  const [postProductSearchTotal, setPostProductSearchTotal] = useState(0)
  ...
  const [customerMessageContent, setCustomerMessageContent] = useState('你好，我想继续咨询。')
  ...
  const selectedDetailImages = useMemo(() => selectedProduct?.detail_images ?? [], [selectedProduct])

  const selectedPostProducts = useMemo(() => {
    const map = new Map<number, ProductListItem>()
    ;[...products, ...postProductCache, ...postProductSearchResults].forEach((product) => map.set(product.id, product))
    if (selectedProduct) map.set(selectedProduct.id, productDetailToListItem(selectedProduct))
    return selectedPostProductIds.map((id) => map.get(id)).filter(Boolean) as ProductListItem[]
  }, [products, postProductCache, postProductSearchResults, selectedPostProductIds, selectedProduct])

  const postProductSearchList = useMemo(() => {
    const map = new Map<number, ProductListItem>()
    postProductSearchResults.forEach((product) => map.set(product.id, product))
    return Array.from(map.values())
  }, [postProductSearchResults])
  ...
  async function searchPostProducts(keyword = postProductSearchKeyword, page = 1, pageSize = postProductSearchPageSize) {
    const trimmedKeyword = keyword.trim()
    setPostProductSearchKeyword(trimmedKeyword)
    setPostProductSearchPage(page)
    setPostProductSearchPageSize(pageSize)
    const data = await run<{ list?: ProductListItem[]; page?: number; page_size?: number; total?: number }>('搜索关联商品', () =>
      productService.listProducts({
    ...
    setPostProductSearchTotal(data?.total ?? list.length)
  ...
  function togglePostProduct(productId: number, checked: boolean) {
    setSelectedPostProductIds((current) => {
      if (!checked) return current.filter((id) => id !== productId)
      return Array.from(new Set([...current, productId]))
    })
  }
  ...
  async function changeCartChecked(item: CartItem, checked: boolean) {
    const data = await run<CartItem[]>(checked ? '勾选购物车商品' : '取消勾选购物车商品', () =>
      orderService.updateCartItem(item.sku_id, { quantity: item.quantity, checked }),
    )
    if (data) {
      setCart(data)
  ...
  async function loadFavoritePosts() {
    if (!authService.hasToken()) return
    const data = await run<{ list?: CommunityFavoritePostItem[] }>('我的收藏帖子', () =>
      communityService.listFavoritePosts({ page_size: 12 }),
    )
  ...
  async function loadCustomerConversations() {
    if (!authService.hasToken()) return
    const data = await run<{ list?: CustomerServiceConversation[] }>('我的客服会话', () =>
      customerService.listConversations({ page_size: 20 }),
    )
    setCustomerConversations(data?.list ?? [])
  }
  ...
  async function openCustomerConversation(conversation: CustomerServiceConversation) {
    ...
    const data = await run<{ list?: CustomerServiceMessage[] }>('客服消息', () =>
      customerService.listMessages(conversation.id, { page_size: 50 }),
    )
    setCustomerMessages(data?.list ?? [])
  }

  function scrollToCustomerService() {
    window.setTimeout(() => {
      document.getElementById('user-customer-service')?.scrollIntoView({ behavior: 'smooth', block: 'start' })
    }, 100)
  }

  async function createPlatformCustomerConversation() {
    const data = await run<CustomerServiceConversation>('进入平台客服会话', () =>
      customerService.createConversation({
        target_type: 'platform',
      }),
    )
    if (data) {
      await loadCustomerConversations()
      await openCustomerConversation(data)
      scrollToCustomerService()
    }
  }

  async function createMerchantCustomerConversation() {
    if (!selectedProduct) {
      api.warning('请先打开一个商品详情，再联系商家客服')
      return
    }
    const data = await run<CustomerServiceConversation>('进入商家客服会话', () =>
      customerService.createConversation({
        target_type: 'merchant',
        merchant_id: selectedProduct.merchant.id,
  ...
  async function sendCustomerMessage() {
    ...
    const data = await run<CustomerServiceMessage>('发送客服消息', () =>
      customerService.sendMessage(selectedCustomerConversation.id, { content: customerMessageContent }),
    )
    if (data) {
  ...
  function syncPostFavoriteState(postId: number, favorited: boolean, favoriteCount: number) {
    const updatePost = (post: CommunityPost) =>
      post.id === postId ? { ...post, favorited, favorite_count: favoriteCount } : post
    setPosts((items) => items.map(updatePost))
    setSelectedPost((post) => (post ? updatePost(post) : post))
    setSelectedCommunityUserPosts((items) => items.map(updatePost))
    setFavoritePosts((items) => {
      if (favorited) return items.map((item) => ({ ...item, post: updatePost(item.post) }))
      return items.filter((item) => item.post.id !== postId)
    })
  }

  async function togglePostFavorite(post: CommunityPost) {
    const data = await run<{ favorited: boolean; favorite_count: number }>(
      post.favorited ? '取消收藏帖子' : '收藏帖子',
      () => communityService.favoritePost(post.id),
    )
    if (data) {
      syncPostFavoriteState(post.id, data.favorited, data.favorite_count)
      if (data.favorited) await loadFavoritePosts()
    }
  }
  ...
  async function logout() {
    ...
    setSelectedCustomerConversation(null)
    setCustomerMessages([])
  ...
  async function openCommunityProductFromPost(post: CommunityPost, productId: number) {
    setSelectedPost(post)
    await openProduct(productId)
  }

  function renderCommunityProductCards(post: CommunityPost, compact = false) {
          ...
          return (
            <Card
              ...
              onClick={(event) => {
                ...
                if (product) void openCommunityProductFromPost(post, product.id)
              }}
            >
              {product ? (
  ...
  }, [categoryId, minPriceYuan, maxPriceYuan, productSort, keyword])
  ...
  return (
    <main className="shop-page">
      ...
      <nav className="workbench-jump-nav" aria-label="用户端联调分区导航">
        ...
        <a href="#user-customer-service">客服</a>
      ...
      <Row gutter={[24, 24]}>
          ...
          <Card
            ...
            extra={
              <Space wrap>
                ...
                />
                <Button onClick={resetProductFilters}>重置筛选</Button>
              </Space>
            }
          >
        ...
        <Col span={8} id="user-cart">
          <Space direction="vertical" size={24} style={{ width: '100%' }}>
                      ...
                      title={(
                        <Space align="start" wrap>
                          <Checkbox
                            ...
                            disabled={Boolean(item.invalid_reason)}
                            onChange={(event) => changeCartChecked(item, event.target.checked)}
                          />
                          <Space direction="vertical" size={4}>
                            <Space wrap>
                              <Text strong>{item.product_name}</Text>
                              {item.source_post_id ? (
                                <Tag color="magenta">种草来源帖子 #{item.source_post_id}</Tag>
                              ) : null}
                            </Space>
                          </Space>
                        </Space>
            ...
            <Card title="客服中心" id="user-customer-service">
              <Space direction="vertical" style={{ width: '100%' }} size={12}>
                <Alert type="info" showIcon message="商家客服只进入对应店铺后台；平台客服只进入平台后台。" />
                <Space wrap>
                  <Button type="primary" onClick={createPlatformCustomerConversation}>进入平台客服会话</Button>
                  <Button onClick={loadCustomerConversations}>刷新会话</Button>
                </Space>
                <Text type="secondary">
                  商家客服入口在商品详情中。进入会话后，在右侧“当前会话消息”里发送咨询内容。
                </Text>
                <Row gutter={[12, 12]}>
                  <Col span={10}>
                    <List
                      ...
                      locale={{ emptyText: '暂无客服会话' }}
                      renderItem={(conversation) => (
                        <List.Item onClick={() => openCustomerConversation(conversation)} className="clickable-list-item">
                          <Space direction="vertical" size={2}>
                            <Space wrap>
                              <Tag color={conversation.target_type === 'platform' ? 'geekblue' : 'purple'}>
                                {conversation.target_type === 'platform' ? '平台' : '商家'}
                              </Tag>
                            ...
                            <Text strong>会话 #{conversation.id}</Text>
                            <Text type="secondary" ellipsis>{conversation.last_message || '暂无消息'}</Text>
                          </Space>
                        </List.Item>
                      )}
                    />
                  ...
                  <Col span={14}>
                    <Card size="small" title={selectedCustomerConversation ? `当前会话 #${selectedCustomerConversation.id}` : '当前会话消息'}>
                      <Space direction="vertical" style={{ width: '100%' }}>
                        <List
                          size="small"
                          dataSource={customerMessages}
                          locale={{ emptyText: '请选择会话查看消息' }}
                          renderItem={(messageItem) => (
                            <List.Item>
                              <Space direction="vertical" size={2}>
                                <Text strong>{statusText(messageItem.sender_type)}：{messageItem.sender_name || `#${messageItem.sender_id}`}</Text>
                                <Text>{messageItem.content}</Text>
                                <Text type="secondary">{new Date(messageItem.created_at).toLocaleString()}</Text>
                              </Space>
                            </List.Item>
                          )}
                        />
                        <Input.TextArea
                          ...
                          placeholder="请选择或进入一个会话后，在这里输入要发送的消息。"
                        />
                        <Button type="primary" disabled={!selectedCustomerConversation || !customerMessageContent.trim()} onClick={sendCustomerMessage}>
                          发送消息
                        </Button>
                      </Space>
                    </Card>
        ...
        <Col span={24} id="user-community">
          ...
          >
                ...
                <Col span={6} key={post.id}>
                  ...
                  >
                    <Space direction="vertical" size={8} style={{ width: '100%' }}>
                      ...
                      {renderCommunityProductCards(post, true)}
                      <Space split={<Divider type="vertical" />}>
                        <Text>赞 {post.like_count}</Text>
                        <Text>藏 {post.favorite_count}</Text>
            ...
            <Card
              size="small"
              title="我的收藏帖子"
              extra={<Button size="small" onClick={loadFavoritePosts}>刷新收藏</Button>}
            >
              {favoritePosts.length === 0 ? (
                <Empty description="还没有收藏帖子" />
              ) : (
                <List
                  size="small"
                  ...
                  renderItem={(item) => (
                    <List.Item
                      actions={[
                        <Button size="small" onClick={() => openPost(item.post)}>查看</Button>,
                        <Button size="small" danger onClick={() => togglePostFavorite(item.post)}>取消收藏</Button>,
                      ]}
                    >
                      <List.Item.Meta
                        title={(
                          <Space wrap>
                            <Tag color={item.post.type === 'grass' ? 'purple' : 'blue'}>{statusText(item.post.type)}</Tag>
                            <Text strong>#{item.post.id} {item.post.title}</Text>
                          </Space>
                        )}
                        description={`收藏于 ${new Date(item.favorited_at).toLocaleString()} / 收藏 ${item.post.favorite_count} / 评论 ${item.post.comment_count}`}
                      />
                    </List.Item>
                  )}
                />
            ...
            <Card size="small" title="发布社区内容">
              <Row gutter={[12, 12]}>
                ...
                <Col span={24}>
                  <Card size="small" className="post-product-picker" title="关联商品">
                    <Space direction="vertical" size={12} style={{ width: '100%' }}>
                      <Text type="secondary">种草帖需选择已完成订单商品；普通帖可以选择任意公开商品。支持商品名搜索，也可以输入商品 ID。</Text>
                      <Input.Search
                        allowClear
                        value={postProductSearchKeyword}
                        onChange={(event) => setPostProductSearchKeyword(event.target.value)}
                        onSearch={(value) => searchPostProducts(value, 1, postProductSearchPageSize)}
                        placeholder="搜索商品名称，或输入商品 ID"
                        enterButton="搜索商品"
                      />
                      {selectedPostProducts.length ? (
                        <Space wrap>
                          <Text strong>已选：</Text>
                          {selectedPostProducts.map((product) => (
                            <Tag
                              ...
                              onClose={(event) => {
                                ...
                                togglePostProduct(product.id, false)
                              }}
                            >
                        ...
                        <Text type="secondary">暂未选择关联商品。</Text>
                      )}
                      <List
                        className="post-product-picker-list"
                        dataSource={postProductSearchList}
                        locale={{ emptyText: '输入关键词搜索商品，或点击搜索加载商品。' }}
                        renderItem={(product) => (
                          <List.Item>
                            <Space align="start" style={{ width: '100%', justifyContent: 'space-between' }}>
                              <Space align="start">
                                <Checkbox
                                  checked={selectedPostProductIds.includes(product.id)}
                                  onChange={(event) => togglePostProduct(product.id, event.target.checked)}
                                />
                                {product.cover_url ? (
                                  <Image width={48} height={48} src={absoluteAssetUrl(product.cover_url)} preview={false} />
                                ) : (
                                  <div className="community-product-thumb">图</div>
                                )}
                                <Space direction="vertical" size={2}>
                                  <Text strong>#{product.id} {product.name}</Text>
                                  <Text type="secondary">店铺 #{product.merchant_id} {product.merchant_name}</Text>
                                </Space>
                              </Space>
                              <Text className="community-product-price">￥{yuan(product.price_cent)}</Text>
                            </Space>
                      ...
                      <Pagination
                        size="small"
                        current={postProductSearchPage}
                        pageSize={postProductSearchPageSize}
                        total={postProductSearchTotal}
                        showSizeChanger
                        pageSizeOptions={[8, 12, 20]}
                        showTotal={(total) => `共 ${total} 个商品`}
                        onChange={(page, pageSize) => searchPostProducts(postProductSearchKeyword, page, pageSize)}
                      />
                    </Space>
                  </Card>
                </Col>
            ...
            <Col span={11}>
              {selectedProductImages[selectedProductImageIndex] ? (
                <Image
                  ...
                  src={absoluteAssetUrl(selectedProductImages[selectedProductImageIndex])}
                  fallback=""
                  preview={false}
                />
              ) : (
              ...
              {selectedProductImages.length ? (
                <Space wrap className="detail-gallery">
                  {selectedProductImages.map((url, index) => (
                    <Button
                      key={`${url}-${index}`}
                      className={selectedProductImageIndex === index ? 'active-thumb-button' : ''}
                      onClick={() => setSelectedProductImageIndex(index)}
                    >
                      <Image preview={false} width={56} height={56} src={absoluteAssetUrl(url)} />
                    </Button>
                  ))}
                </Space>
              ) : null}
            ...
            <Col span={13}>
              <Space direction="vertical" size={18} style={{ width: '100%' }}>
                <Space wrap>
                  ...
                  <Tag color="green">销量 {selectedProduct.sales_count}</Tag>
                </Space>
                ...
                <Card size="small" title="联系商家客服">
                  <Space direction="vertical" style={{ width: '100%' }}>
                    <Text type="secondary">
                      咨询店铺 #{selectedProduct.merchant.id} {selectedProduct.merchant.name}，会话只会出现在该商家的“本店客服”中。
                    </Text>
                    <Button type="primary" onClick={createMerchantCustomerConversation}>
                      进入该商品商家会话
                    </Button>
                    <Text type="secondary">进入会话后，到客服中心的当前会话区域发送咨询内容。</Text>
                  </Space>
                </Card>
        ...
        {selectedPost ? (
          <Space direction="vertical" size={16} style={{ width: '100%' }}>
            ...
            <Card size="small" title="关联商品">
              {renderCommunityProductCards(selectedPost)}
            </Card>
            <Space>
              <Button onClick={() => communityService.likePost(selectedPost.id).then(() => loadPosts())}>
                点赞 {selectedPost.like_count}
              </Button>
              <Button type={selectedPost.favorited ? 'primary' : 'default'} onClick={() => togglePostFavorite(selectedPost)}>
                {selectedPost.favorited ? '已收藏' : '收藏帖子'} {selectedPost.favorite_count}
              </Button>
              {selectedPost.product_ids.map((productId) => (
                <Button key={productId} onClick={() => openCommunityProductFromPost(selectedPost, productId)}>
                  查看商品 #{productId}
                </Button>
              ))}
            </Space>
