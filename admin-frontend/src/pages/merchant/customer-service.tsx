...
export function MerchantCustomerServicePage() {
  ...
  const [customerMessageContent, setCustomerMessageContent] = useState('商家客服已收到。')
  ...
  async function loadCustomerConversations() {
    const data = await run<PageResult<CustomerServiceConversation>>('本店客服会话', () =>
  ...
  async function openCustomerConversation(conversation: CustomerServiceConversation) {
    ...
    const data = await run<PageResult<CustomerServiceMessage>>('本店客服消息', () =>
  ...
  async function sendCustomerMessage() {
    ...
    const data = await run<CustomerServiceMessage>('回复本店客服', () =>
  ...
  async function closeCustomerConversation() {
    if (!selectedCustomerConversation) return
    const data = await run<CustomerServiceConversation>('关闭本店客服会话', () =>
  ...
  return (
    <main className="admin-page">
      ...
      <Card title="本店客服" extra={<Button onClick={loadCustomerConversations}>刷新会话</Button>}>
        <Alert type="info" showIcon message="商家端只处理本店客服会话，平台客服会话不会出现在这里。" style={{ marginBottom: 12 }} />
        <Row gutter={[16, 16]}>
          <Col span={9}>
            <List
              dataSource={customerConversations}
              locale={{ emptyText: '暂无本店客服会话' }}
              renderItem={(conversation) => (
                <List.Item onClick={() => openCustomerConversation(conversation)} className="clickable-list-item">
                  <Space direction="vertical" size={2}>
                    <Space wrap>
                      <Tag color="purple">本店</Tag>
                    ...
                    {conversation.product_name ? <Text type="secondary">商品：{conversation.product_name}</Text> : null}
                    {conversation.order_no ? <Text type="secondary">订单：{conversation.order_no}</Text> : null}
          ...
          <Col span={15}>
            <Card size="small" title={selectedCustomerConversation ? `本店会话 #${selectedCustomerConversation.id}` : '客服消息'}>
